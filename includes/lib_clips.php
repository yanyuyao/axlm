...
function insert_pay_log($id, $amount, $type = PAY_SURPLUS, $is_paid = 0)
{
    ...
    $GLOBALS['db']->query($sql);
     ...
     return $GLOBALS['db']->insert_id();
	 */
	$sql = "select log_id from ".$GLOBALS['ecs']->table('pay_log')." where order_id = '$id'";
	$log_id = $GLOBALS['db']->getOne($sql);
	if($log_id){
		$sql = "update ".$GLOBALS['ecs']->table('pay_log')." set order_amount = '$amount', order_type = '$type', is_paid = '$is_paid' where log_id = '$log_id'";
		$GLOBALS['db']->query($sql);
		return $log_id;
	}else{
		$sql = 'INSERT INTO ' .$GLOBALS['ecs']->table('pay_log')." (order_id, order_amount, order_type, is_paid)".
...
/*代码修改2014-12-23 by www.cfweb2015.com  _star */
    $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
/*代码修改2014-12-23 by www.cfweb2015.com  _end */
    $row = $GLOBALS['db']->getRow($sql);
