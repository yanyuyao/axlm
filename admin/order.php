...
/*------------------------------------------------------ */
//-- start一键发货 增加 by www.cfweb2015.com
/*------------------------------------------------------ */
...
/*------------------------------------------------------ */
//-- end一键发货 增加 by www.cfweb2015.com
/*------------------------------------------------------ */
    /* 付款 */
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('pay' == $operation)
    {
        ...
        update_order($order_id, $arr);
		/* 已付款，则 参与返现 */
		$order_yingfukuan = $GLOBALS['db']->getOne("select order_amount from ".$GLOBALS['ecs']->table('pay_log')." where order_id = ".$order['order_id']);
		printsqls("select order_amount from ".$GLOBALS['ecs']->table('pay_log')." where order_id = ".$order['order_id']);
		user_back_point($order['user_id'], $order_yingfukuan);
		
		expend_amount_order_log($order['order_id'], $order['user_id']);
					
					
        /* 记录log */
        order_action($order['order_id'], OS_CONFIRMED, $order['shipping_status'], PS_PAYED, $action_note);
    }
    /* 设为未付款 */
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        if($_REQUEST['act'] == 'invoice_list' || (isset($_REQUEST['act_detail'])&&$_REQUEST['act_detail']=='invoice_query'))
        {
         ...
         /* 普通订单不显示虚拟团购订单 添加_START_www.cfweb2015.com*/
        $where .= " AND o.extension_code != 'virtual_good'";
         /* 普通订单不显示虚拟团购订单 添加_END_www.cfweb2015.com*/
		/*增值税发票_添加_END_www.cfweb2015.com*/
		/* 代码增加_start   By www.ecshop68.com */
