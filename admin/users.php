...
function action_insert ()
{
	...
	$sql = "update " . $ecs->table('users') . " set  mobile_phone='$mobile_phone' , `real_name`='$real_name',`card`='$card',`country`='$country',`province`='$province',`city`='$city',`district`='$district',`address`='$address',`status`='$status', huiyuanka='$huiyuanka' where user_name = '" . $username . "'";
	$db->query($sql);
...
function action_edit ()
{
	...
	$sql = "SELECT u.user_name, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn, u.office_phone, u.home_phone, u.mobile_phone,u.huiyuanka " . " FROM " . $ecs->table('users') . " u LEFT JOIN " . $ecs->table('users') . " u2 ON u.parent_id = u2.user_id WHERE u.user_id='$_GET[id]'";
	...
	/* 代码增加2014-12-23 by www.cfweb2015.com _star */
	$sql = "SELECT u.user_id, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank ,u.huiyuanka, u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn,
...
function action_update ()
{
	...
	$sql = "update " . $ecs->table('users') . " set `real_name`='$real_name',`card`='$card',`country`='$country',`province`='$province',`city`='$city',`district`='$district',`address`='$address',`status`='$status' ,huiyuanka = '$huiyuanka' where user_name = '" . $username . "'";
	$db->query($sql);
...
function user_list ()
{
	...
	if($result === false)
	{
		...
		// reg_time ".
		$sql = "SELECT user_id, user_name, email, mobile_phone, is_validated, validated, user_money, frozen_money, rank_points, pay_points, status, reg_time, froms, huiyuanka ".
		        /* 代码增加2014-12-23 by www.cfweb2015.com  _end  */
