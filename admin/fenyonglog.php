...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('fenyonglog.htm'), '', array('filter' => $logdb['filter'], 'page_count' => $logdb['page_count']));
}
...
function get_list($user_id)
{
    ...
    $sqladd = ' and ue.expend_user_id = '.$user_id;
  
    if(!empty($config_data['on']))
    {
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('user_expend_log') . " ue ".
				" left join ".$GLOBALS['ecs']->table('users')." u on ue.expend_user_id = u.user_id ".
    ...
    if(!empty($config_data['on']))
    {
            ...
            //推荐订单分成
            $sql = "SELECT u.user_name, ue.id, ue.user_id, ue.order_id, ue.expend_bili,ue.expend_level, ue.expend_user_id, ue.order_amount, ue.expend_amount,ue.status, ue.ctime " .
    ...
    {
			$rt['expend_level'] = $rt['expend_level']?$rt['expend_level']."级":'';
			$rt['expend_amount_format'] = price_format($rt['expend_amount']);
			$rt['expend_bili'] = $rt['expend_bili']?$rt['expend_bili']."%":'';
			$rt['status'] = $rt['status'] != 1?"审核中":"已审核";
