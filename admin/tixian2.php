...
{
    /* 权限判断 */
    admin_priv('tixian_manage');

    $smarty->assign('ur_here',       "提现处理");
   // $smarty->assign('action_link',   array('text' => ‘’, 'href'=>'user_account.php?act=add'));
    ...
    assign_query_info();
    $smarty->display('user_tixian_list.htm');
}else if($_REQUEST['act'] == 'payment'){
	
	if($_POST['checkboxes']){
		$ids = db_create_in($_POST['checkboxes']);
		$sql = "select id, user_id, amount, daozhuang_amount,add_time, tixian_account_info, tixian_account_realname, is_paid from ".$GLOBALS['ecs']->table('user_account')." where is_paid = 0 AND tixian_account_realname != '' AND tixian_account_info != '' AND id ".$ids;
		//echo $sql;
		$recordlist = $GLOBALS['db']->getAll($sql);
		//var_dump($record);
		$pay_date = date('Ymd',time());
		$max = $GLOBALS['db']->getOne("select max(id) from ".$GLOBALS['ecs']->table('alipy_pay'));
		$xuliehao = sprintf("%010d", intval($max+1));
		$picihao = $pay_date.$xuliehao;
		$record = format_tixian_user_account($picihao, $recordlist);
		$sql = "insert into ".$GLOBALS['ecs']->table('alipy_pay')."(paydate, xuliehao,picihao,amount,paynum, detail, `status` ,ctime)values("
				."'".$pay_date."',"
				."'".$xuliehao."',"
				."'".$picihao."',"
				."'".$record['pay_amount']."',"
		...
		foreach($recordlist as $k=>$v){
			...
			$id_sql[] = "(".$picihao.",".$v['id'].",".$v['user_id'].")";
			$id_sql_str = implode(",",$id_sql);
		}
		$sql = "insert into ".$GLOBALS['ecs']->table('alipy_pay_tixian')."(picihao,user_account_id,user_id)values ".$id_sql_str;
		//echo $sql;
		$GLOBALS['db']->query($sql);
	
		$sql = "update ".$GLOBALS['ecs']->table('user_account')." set push_to_alipay_time = '".time()."' where id ".$ids;
		$GLOBALS['db']->query($sql);
		
		//var_dump($record);
/***================= end 支付宝提现 ===================***/	
require_once(dirname(__FILE__) . "/zhifubao/alipay.config.php");
require_once(dirname(__FILE__) . "/zhifubao/lib/alipay_submit.class.php");

/**************************请求参数**************************/

        //服务器异步通知页面路径
        $notify_url = "http://www.mdzyz.cn/piliang_notify_url.php";
        //需http://格式的完整路径，不允许加?id=123这类自定义参数

        //付款账号
        //$email = $_POST['WIDemail'];
        $email = "beijingpinrui@126.com";
        //必填

        //付款账户名
        //$account_name = $_POST['WIDaccount_name'];
        $account_name = "北京品瑞珠宝有限责任公司";
        ...
        $batch_fee = $record['pay_amount'];
        //必填，即参数detail_data的值中所有金额的总和

        //付款笔数
        //$batch_num = $_POST['WIDbatch_num'];
        $batch_num = $record['pay_num'];;
        //必填，即参数detail_data的值中，“|”字符出现的数量加1，最大支持1000笔（即“|”字符出现的数量999个）

        //付款详细数据
        //$detail_data = $_POST['WIDdetail_data'];
        $detail_data = $record['detail'];
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('user_tixian_list.htm'), '', array('filter' => $list['filter'], 'page_count' => $list['page_count']));
}
/*------------------------------------------------------ */
//-- ajax删除一条信息
/*------------------------------------------------------ */
...
* - 修改用户余额
*/
function update_tixian_user_account($picihao, $record){
	$ids = array();
	foreach($record as $k=>$v){
		...
		$sql = "insert into ".$GLOBALS['ecs']->table('alipy_pay_tixian')."(picihao,user_account_id)values ".$id_sql_str;
		$GLOBALS['db']->table($sql);
		
		//更新会员余额数量
        log_account_change($v['user_id'], $v['amount'], 0, 0, 0, '批量体现', 5);
	}
	if($ids){
		$sql = "update ".$GLOBALS['user_account']." set is_paid = 1 where id in (".implode(",",$ids).") ";
		$GLOBALS['db']->query($sql);
	}
}
...
* pay_num : 付款笔数
*/
function format_tixian_user_account($picihao, $record){
	$list = array();
	$data = array();
	$pay_num = 0;
	$pay_amount = 0;
	foreach($record as $k=>$v){
		$user_id = $v['user_id'];
		//$money = -$v['amount'];
		$money = -$v['daozhuang_amount'];
		$realname = $v['tixian_account_realname'];
		$account_info = $v['tixian_account_info'];
		$note = "商城提现";
		//$liushuihao = $v['add_time']; //申请时间当作流水号
		$liushuihao = sprintf("%010d", $v['id']);
		$list[] = $liushuihao."^".$account_info."^".$realname."^".$money."^".$note;
...
function tixian_list()
{
    ...
    if ($result === false)
    {
        ...
        $filter['paid_status'] = isset($_REQUEST['paid_status']) ? intval($_REQUEST['paid_status']) : 'all';
       ...
       // $filter['end_date'] = empty($_REQUEST['end_date']) ? '' : (local_strtotime($_REQUEST['end_date']) + 86400);
        ...
        $where = " WHERE 1 ";
		if($filter['paid_status'] == 'all'){
			
		}elseif($filter['paid_status'] == "1"){ //已完成
			$where .= ' AND is_paid = "1" ';
		}elseif($filter['paid_status'] == "3"){ //未完成
			$where .= ' AND is_paid = "0" ';
		}elseif($filter['paid_status'] == "2"){ //操作中
			$where .= ' AND push_to_alipay_time is not null AND alipay_return is null ';
		}
       ...
       */
		...
		*/
        ...
        $sql  = 'SELECT ua.*, u.user_name FROM ' .
            ...
            $where . "ORDER by ua.id desc LIMIT ".$filter['start'].", ".$filter['page_size'];
    ...
    {
		if($value['push_to_alipay_time'] && $value['alipay_return'] == 'success'){
			$list[$key]['alipay_return'] = "操作成功";
		}elseif($value['push_to_alipay_time'] && $value['alipay_return'] == 'fail'){
			$error_msg = $value['alipay_error_msg']?" : ".$value['alipay_error_msg']:"";
			$list[$key]['alipay_return'] = "操作失败".$error_msg;
		}else if($value['push_to_alipay_time']){
			$list[$key]['alipay_return'] = "处理中";
		}else{
			$list[$key]['alipay_return'] = "";
		}
		
        $list[$key]['surplus_amount']       = price_format(abs($value['amount']), false);
        $list[$key]['daozhuang_amount']       = price_format(abs($value['daozhuang_amount']), false);
        $list[$key]['add_date']             = local_date($GLOBALS['_CFG']['time_format'], $value['add_time']);
        $list[$key]['alipay_exec_time']             = local_date($GLOBALS['_CFG']['time_format'], $value['alipay_exec_time']);
