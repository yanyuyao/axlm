...
{
    clear_all_files();
    clearhtml_all();  //代码增加   By  www.cfweb2015.com
    echo 1;
    //sys_msg($_LANG['caches_cleared']);
}
//www.cfweb2015.com手机缓存修改start
...
elseif ($_REQUEST['act'] == 'main')
{
    ...
    /*后台管理起始页_修改_START_www.cfweb2015.com*/
...
//今日注册会员
    ...
    $task['commission'] = $db->getOne('SELECT COUNT(*) FROM '.$ecs->table('user_expend_log').' WHERE status=0');
    //待审核商品
    // 代码修改   By  www.cfweb2015.com Start
...
//    " WHERE supplier_id=0  " . order_query_sql('await_pay'));
    ...
    $order['new_repay'] = $db->getOne('SELECT COUNT(*) FROM '.$ecs->table('back_order').' WHERE status_back < 6 AND status_back != 3 AND back_type = 4 AND supplier_id=0');// 代码修改   By  www.cfweb2015.com End
    //退货申请
    // 代码修改   By  www.cfweb2015.com Start
...
//    " WHERE supplier_id=0 " . order_query_sql('finished');
    ...
    /*后台管理起始页_修改_END_www.cfweb2015.com*/
