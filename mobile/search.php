...
else
{
	...
	$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'final_price', 'last_update', 'salenum'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;   /* 代码增加_start  By  www.cfweb2015.com */
    ...
    if (!empty($_REQUEST['intro']))
    {
        switch ($_REQUEST['intro'])
        {
			...
			case 'zlyc': //助老残疾
                 $time    = gmtime();
                $intro   = " AND gtt.tag_name = '养老助残' ";
                $intromode = 'zlyc';
				$sql_left = " LEFT JOIN ".$GLOBALS['ecs']->table('goods_tag')." gtt ON gtt.goods_id = g.goods_id ";
                $ur_here = '养老助残';
                break;
			case 'fnet': // 妇女儿童
                $time    = gmtime();
                $intro   = " AND gtt.tag_name = '关爱妇女儿童' ";
                $intromode = 'fnet';
				$sql_left = " LEFT JOIN ".$GLOBALS['ecs']->table('goods_tag')." gtt ON gtt.goods_id = g.goods_id ";
                $ur_here = '关爱妇女儿童';
                break;
