	...
	if(array_search(0,$supplier_ids) !== false){
		if ($position == 1){
			if ($_CFG['sms_order_placed'] == '1' && $_CFG['sms_shop_mobile'] != '')
		    {
				...
				$content1 = "【品瑞嘉】".$content1;
				fwrite($fp, "content1: ".$content1."\r\n");
...
function set_fenxiao_code()
{
    ...
    {
        setcookie('fx_ucode', $_GET['ucode'], gmtime() + 3600 * 24); // 过期时间为 1 天
        /*
        if(!empty($config['config']['expire']))
        {
            ...
            setcookie('fx_ucode', intval($_GET['ucode']), gmtime() + 3600 * $config['config']['expire'] * $c);
        }
        else
        {
            setcookie('fx_ucode', intval($_GET['ucode']), gmtime() + 3600 * 24); // 过期时间为 1 天
        }
        */
    }
}
...
function get_user_base_expend_code($uid){
    ...
    if($row){
		...
		$row['expend_amount_rest_format'] = price_format($row['expend_amount_rest']);
...
function set_user_bonus_level($id){
	...
	$zhitui_leve1_l1 =  $zhitui_leve1['sbs1_leve1'];
	
	$zhitui_leve2_num = $zhitui_leve1['sbs2_share_people_num'];
	$zhitui_leve2_l1 =  $zhitui_leve1['sbs2_leve1'];
	
	$zhitui_leve3_num = $zhitui_leve1['sbs3_share_people_num'];
	$zhitui_leve3_l1 =  $zhitui_leve1['sbs3_leve1'];
	
	$zhitui_leve4_num = $zhitui_leve1['sbs4_share_people_num'];
	$zhitui_leve4_l1 =  $zhitui_leve1['sbs4_leve1'];
	
	$zhitui_leve5_num = $zhitui_leve1['sbs5_share_people_num'];
	$zhitui_leve5_l1 =  $zhitui_leve1['sbs5_leve1'];
	// 计算有效的总推广人数 
...
//确认订单后，设置父级为有效用户
function set_user_to_fenxiao($userid){ //用户注册时， 设置其父级成为分销会员、
...
function expend_order_log($order_id, $user_id){
    ...
    if($order && $order['goods_amount']){
        ...
        //var_dump($epuid_l1);
        if($epuid_l1 && $epuid_l1['level']){ //达到分销级别了，计算分佣
...
function get_bonus_list($user_id,$num){
	...
	if($data){
		foreach($data as $k=>&$v){
			...
			$v['bonus_money_format'] = price_format($v['bonus_money']);
		}
	}
	return $data;
}
...
function get_recommend_shops($type=1){
	$where = " WHERE 1 ";
	$where .= " AND s.status = 1 AND s.is_home = 1 ";
	...
	if($type == 1){
		...
		$list = $GLOBALS['db']->getAll($sql." limit 6 ");
		return $list;
	}elseif($type == 2){
		
		$list = $GLOBALS['db']->getAll($sql." limit 7 ");
		$data = array();
		if($list){
			foreach($list as $k=>$v){
				if($k == 0){
...
function expend_amount_order_log($order_id, $user_id){
	...
	$order_amount = $GLOBALS['db']->getOne("select order_amount from ".$GLOBALS['ecs']->table('pay_log')." where order_id = ".$order['order_id']);
	
	
	$level365 =  $fenxiao['dataitem']['zhitui_leve1'];
	$level3650 =  $fenxiao['dataitem']['zhitui_leve2'];
	...
	if($order_amount>=$level365_money){
		...
		if($fenxiao_level){
			$sql = "update fenxiao_level = '$fenxiao_level', is_valid_user = 1,is_expend_valid = 1 and buyctime='".time()."' where user_id = '$user_id'";
			$GLOBALS['db']->query($sql);
		}
